...
export interface IHibitClient {
  ...
  /**
   * Get the chain balance for an asset.
   *
   * @param {bigint} assetId - The asset id to get the chain balance for.
   * @returns {Promise<bigint>} A promise that resolves to the chain balance in the smallest unit of the asset.
   */
  getChainBalance(assetId: bigint): Promise<bigint>;
...
export class HibitClient implements IHibitClient {
  ...
  async getChainBalance(assetId: bigint): Promise<bigint> {
    const apiName = 'getChainBalance';
    const response = await getV1AssetChainBalance(mapGetAssetInput(assetId));
