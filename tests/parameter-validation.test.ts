...
describe('HibitClient Parameter Validation Tests', () => {
  ...
  describe('Error type validation', () => {
    it('should throw HibitError instances', async () => {
      try {
        await client.resetProxyKey(undefined as any);
      } catch (error) {
        expect(error).toBeInstanceOf(HibitError);
        expect(error.name).toBe('HibitError');
      }
    });
  });
  ...
  describe('getOrders HIN parameter behavior', () => {
    it('should use client options.hin when hin is not provided in input', async () => {
      ...
      // We expect this to pass validation and make the API call
      try {
        await client.getOrders({
          limit: 10,
          offset: 0
        });
        // If we reach here, the validation passed (expected behavior)
        expect(true).toBe(true);
      } catch (error: any) {
        // Should only fail due to API call, not parameter validation
        expect(error.name).toBe('HibitError');
        expect(error.message).not.toContain('Missing required parameter');
      }
    });
  ...
  describe('getAsset parameter validation', () => {
    ...
    it('should not throw validation error when assetId is provided', async () => {
      // This test verifies that the validation passes (the API call itself may succeed or fail)
      try {
        await client.getAsset({ assetId: 123n });
        // If it succeeds, that's fine
      } catch (error: any) {
        // If it throws an error, it should not be our validation error
        expect(error.message).not.toContain('either assetId or tokenAddress must be provided');
      }
    });
    ...
    it('should not throw validation error when tokenAddress is provided', async () => {
      // This test verifies that the validation passes (the API call itself may succeed or fail)
      try {
        await client.getAsset({ tokenAddress: '0x123' });
        // If it succeeds, that's fine
      } catch (error: any) {
        // If it throws an error, it should not be our validation error
        expect(error.message).not.toContain('either assetId or tokenAddress must be provided');
  ...
  describe('cancelSpotOrder parameter validation', () => {
    ...
    it('should not throw validation error for scenario 1: cancel by orderId', async () => {
      // This test verifies that the validation passes (the API call itself may succeed or fail)
      try {
        await client.cancelSpotOrder({ orderId: 'valid-order-id' });
        // If it succeeds, that's fine
      } catch (error: any) {
        // If it throws an error, it should not be our validation error
        expect(error.message).not.toContain('either orderId must be provided');
      }
    });
